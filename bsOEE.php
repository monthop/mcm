...
.tableFixHead th {
  ...
  background: #eee;
				...
				<div class='table-responsive'>
					...
					<table border='1' class='table table-hover table-inverse table-bordered table-sm '>  
						...
						<?php
							...
							$sql = "SELECT * FROM mcmonitoring.plandata WHERE ID = $ID and McNo = $McNoStr;";
							...
							if ($result->num_rows > 0) {
								while($row = mysqli_fetch_array($result))
								{
									...
									$RunTime = $row['RunDateTime'];
									$StopTime = $row['StopDateTime'];

									$AQty = $row['AQty'];
									$AvTime = $row['AvTime'];
									$CT = $row['CycleTime'];
									$Defect = $row['TotalDefect'];
									//echo "Data received";
									echo "<tr>";
									echo "<td>".$row['ID']."</td>";
	...
	<a href="/bsexportOEE.php" class="btn btn-warning" role="button">Export OEE File</a>
	<a href="/bscalOEE-001.php" class="btn btn-info" role="button">Go Back</a>
</div>
